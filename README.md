...
This usually means `node_modules` was copied from another operating system or
lost executable permissions. Run `npm install` on the current machine. On macOS
or Linux, the local command shims may also need executable permissions:
...
Stop duplicate development servers, remove the generated `.next` directory,
and start one fresh process:
...
The project uses `next dev --webpack` because Payload's wrapper disables the
Turbopack Fast Refresh path used by this setup.

### Grammarly hydration warning

Attributes such as `data-gr-ext-installed` or
`data-new-gr-c-s-check-loaded` are injected by the Grammarly browser extension
before React hydrates. They are not generated by the application. Disable the
extension for localhost or test in a private window to confirm.

## Production notes

- Replace the SQLite adapter with a production database adapter such as
  PostgreSQL before deploying a multi-instance production environment.
- Set a strong `PAYLOAD_SECRET` and production `DATABASE_URI`.
- Set `NEXT_PUBLIC_SITE_URL` to the canonical HTTPS website URL.
- Configure a persistent storage solution for uploaded Media.
